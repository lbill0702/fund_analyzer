...
def build_performance_chart(
    ...
    if show_benchmark and not combined.empty:
        ...
        # Build a daily cumulative index from the annual returns dict.
        # Strategy: for each calendar year present in MSCI_ACWI_MINVOL, spread
        # the annual return linearly across business days in that year.
        bdays = pd.bdate_range(
            start=pd.Timestamp(date_min.year, 1, 1),
            end=pd.Timestamp(date_max.year, 12, 31),
        )
        bm_series = pd.Series(index=bdays, dtype=float)
        cumulative = 1.0
        for year in range(date_min.year, date_max.year + 1):
            annual_ret = MSCI_ACWI_MINVOL.get(year)
            if annual_ret is None:
                # No data for this year — carry flat
                yr_days = bdays[(bdays.year == year)]
                for d in yr_days:
                    bm_series[d] = cumulative
                continue
            yr_days = bdays[bdays.year == year]
            if len(yr_days) == 0:
            ...
            vals = np.linspace(start_cum, end_cum, len(yr_days) + 1)[1:]
            for d, v in zip(yr_days, vals):
                bm_series[d] = v
            cumulative = end_cum

        bm_series = bm_series.dropna()
        bm_series = bm_series[(bm_series.index >= date_min) & (bm_series.index <= date_max)]

        # Normalise so benchmark starts at 1.0 on the same first date as the data
...
with st.sidebar:
    ...
    show_benchmark = st.checkbox(
        ...
        value=True,
        help="Overlay MSCI ACWI Min Volatility Index on the performance line chart and calendar bar chart.",
    )
...
with tab_div:
    if not all_divs:
        ...
        for fund in combined_div_all["Fund_Name"].unique():
            ...
            # TTM Distribution Yield = sum(last 12 payments) / ending NAV
            # Matches BlackRock's published Distribution Yield methodology
            ...
            latest_yield = (trailing12 / nav_on_latest_div) if not np.isnan(nav_on_latest_div) else None
            ...
            with col2:
                ...
                nav_str = f"NAV {nav_on_latest_div:.4f}" if not np.isnan(nav_on_latest_div) else "—"
                st.markdown(
                    f"<div class='stat-card info'>"
                    f"<div class='label'>TTM Distribution Yield</div>"
                    f"<div class='value' style='color:{color}'>{yield_str}</div>"
                    f"<div class='meta'><span style='color:var(--text3)'>{nav_str}</span></div>"
                    f"</div>",
                    unsafe_allow_html=True,
                )
            with col3:
        ...
        with st.expander("Full Dividend History Table"):
            ...
            # TTM Distribution Yield per row: rolling sum of 12 payments / NAV
            # Matches BlackRock's published Distribution Yield methodology
            def _rolling_ttm_yield(group: pd.DataFrame) -> pd.Series:
                group = group.sort_values("Ex_Date").reset_index(drop=True)
                ttm = group["Distribution"].rolling(window=12, min_periods=1).sum()
                return (ttm / group["Price"]).where(group["Price"].notna())

            display_div["Ann_Yield"] = (
                display_div.groupby("Fund_Name", group_keys=False)
                .apply(_rolling_ttm_yield)
            ...
            st.dataframe(
                ...
                .rename(columns={
                    ...
                    "Distribution_str": "Distribution", "NAV_str": "Unit Price", "Yield_str": "TTM Dist. Yield"
                })
