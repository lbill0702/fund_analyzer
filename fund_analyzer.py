...
def build_performance_chart(
    ...
    # ── MSCI ACWI Min Vol benchmark overlay ──────────────────────────────────
    if show_benchmark and not combined.empty:
        date_min = combined["Date"].min()
        date_max = combined["Date"].max()
        # Try live daily ACWV prices first; fall back to annual interpolation
        series: Optional[pd.Series] = None
        live = _fetch_ticker_daily("ACWV")
        if live is not None and not live.empty:
            clipped = live[(live.index >= date_min) & (live.index <= date_max)]
            if not clipped.empty:
                series = clipped / clipped.iloc[0]
        if series is None:
            series = _annual_to_daily(MSCI_ACWI_MINVOL, date_min, date_max)
        if series is not None:
            fig.add_trace(go.Scatter(
                x=series.index, y=series.values,
                mode="lines", name="MSCI ACWI Min Vol",
                line=dict(color=CHART_BM_COLOR, width=1.8, dash="dash"),
                opacity=0.75,
                hovertemplate=(
...
def _fetch_ticker_daily(ticker: str) -> Optional[pd.Series]:
    """Fetch daily adjusted-close prices via yfinance; returns None on failure."""
    try:
...
with st.sidebar:
    ...
    show_benchmark = st.checkbox(
        "Benchmark (MSCI ACWI Min Vol)",
        value=True,
        help="Overlay MSCI ACWI Min Volatility Index on the performance chart and calendar bar chart.",
    )
...
with tab_div:
    if not all_divs:
        ...
        with st.expander("Full Dividend History Table"):
            ...
            # Monthly Yield % = distribution ÷ NAV × 12
