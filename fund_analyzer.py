...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@300;400;500;600;700&family=JetBrains+Mono:wght@400;500&family=Fraunces:ital,wght@0,300;0,600;1,300&display=swap');
...
.stat-card.positive::before { background: linear-gradient(90deg, transparent, var(--positive), transparent); opacity: 0.5; }
.stat-card.negative::before { background: linear-gradient(90deg, transparent, var(--negative), transparent); opacity: 0.5; }
.stat-card .label {
  font-size: 0.65rem;
  font-weight: 600;
  text-transform: uppercase;
...
/* ── Sort buttons — compact ghost style ── */
.stButton button {
  background: transparent !important;
  color: var(--text2) !important;
...
with st.sidebar:
    ...
    st.markdown(
        ...
        "<div class='wordmark'>Fund Analyzer</div>"
    ...
    st.markdown(
        ...
        "<span class='sidebar-section-label'>Data Sources</span>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='sidebar-section'>"
        "<span class='sidebar-section-label'>Chart Overlays</span>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='sidebar-section'>"
        "<span class='sidebar-section-label'>View Options</span>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        "<p>Auto-detects CSV/Excel headers · English &amp; Chinese · "
        "Volatility = σ × √freq</p>"
        "</div></div>",
        unsafe_allow_html=True,
    )
...
st.markdown(
    ...
    "<div>"
    "<h1>Fund Performance Analyzer<span class='dot'></span></h1>"
    "</div>"
    "<div class='subtitle'>Multi-fund comparison&ensp;·&ensp;"
    "Black Swan overlays&ensp;·&ensp;Rolling analytics&ensp;·&ensp;Calendar returns</div>"
    "</div>",
    unsafe_allow_html=True,
)
...
if not uploaded_files:
    st.markdown(
        ...
        "stroke='#4a6070' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'>"
        ...
        "</svg>"
        "</div>"
        "<h3>Drop your fund files here</h3>"
        "<p>Accepts CSV and Excel — Manulife, BlackRock SpreadsheetML,<br>"
        "Allianz, and any format with date + price columns.<br>"
        "English and Chinese headers auto-detected.</p>"
        "</div>",
        unsafe_allow_html=True,
    )
    st.stop()
            ...
            for i, ev in enumerate(row_events):
                with cols_bs[i]:
                    ...
                    st.markdown(
                        ...
                        f"<div class='bs-date' style='color:{ev['label_color']};'>{date_str}</div>"
                        f"</div>",
                        unsafe_allow_html=True,
                    )
