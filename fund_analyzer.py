...
# ─────────────────────────────────────────────────────────────────────────────
def build_dividend_chart(all_divs: list, fund_color_map: dict) -> go.Figure:
    """
    Dual-panel dividend chart:
      Top:    Bar chart of per-payment distribution amount over time
      Bottom: Annual total distribution (stacked by fund if multiple)
    """
    if not all_divs:
        return None
    ...
    fig = make_subplots(
        ...
        subplot_titles=("Monthly Distribution per Unit (USD)", "Annual Total Distribution (USD)"),
    )

    # ── Top panel: monthly bars per fund ─────────────────────────────────────
    for fund in funds:
    ...
    # Rolling 12-month yield overlay (distribution / NAV * 100)
    # Use a 12-period rolling sum of distributions as trailing yield proxy
    for fund in funds:
        fd = combined_div[combined_div["Fund_Name"] == fund].sort_values("Ex_Date")
        fd = fd.set_index("Ex_Date").resample("ME")["Distribution"].sum().reset_index()
        fd["Trailing12m"] = fd["Distribution"].rolling(12, min_periods=6).sum()
        color = fund_color_map.get(fund, "#00d4aa")
        fig.add_trace(
            go.Scatter(
                x=fd["Ex_Date"],
                ...
                hovertemplate=(
                    "<b>%{fullData.name}</b><br>"
                    "Date: %{x|%b %Y}<br>"
                    "Trailing 12m total: <b>USD %{y:.4f}</b><extra></extra>"
                ),
                legendgroup=fund,
                showlegend=False,
                yaxis="y",
    ...
    # ── Bottom panel: annual totals bar ──────────────────────────────────────
    for fund in funds:
        fd = combined_div[combined_div["Fund_Name"] == fund]
        annual = fd.groupby("Year")["Distribution"].sum().reset_index()
        # Exclude partial years at boundaries (< 10 months)
        count_by_year = fd.groupby("Year")["Distribution"].count()
        full_years = count_by_year[count_by_year >= 10].index
        annual_full = annual[annual["Year"].isin(full_years)]
        color = fund_color_map.get(fund, "#00d4aa")
        fig.add_trace(
            go.Bar(
    ...
    # Annotation for dotted line
    fig.add_annotation(
        text="― ― trailing 12m total",
        xref="paper", yref="paper",
        x=1.0, y=0.98,
        showarrow=False,
        font=dict(size=10, color="#6b7f90", family="DM Mono"),
        align="right",
    )
    return fig
...
with tab_div:
    if not all_divs:
        ...
        # ── Dividend chart ────────────────────────────────────────────────────
        div_fig = build_dividend_chart(all_divs, fund_color_map)
        if div_fig:
        ...
        with st.expander("Full Dividend History Table"):
            ...
            display_div["Ex_Date"] = display_div["Ex_Date"].dt.strftime("%d %b %Y")
            display_div["Distribution"] = display_div["Distribution"].map("USD {:.4f}".format)
            st.dataframe(
                display_div[["Fund_Name", "Ex_Date", "Year", "Distribution"]]
