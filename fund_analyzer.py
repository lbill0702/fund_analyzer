...
def _best_match(columns: list[str], keywords: list[str]) -> str | None:
...
# ─────────────────────────────────────────────────────────────────────────────
def load_and_normalize(uploaded_file) -> tuple[pd.DataFrame | None, str]:
    """
    Parse one uploaded CSV file into the standard long-format DataFrame.
    ...
    # Fund name = filename without extension
    ...
    detected_msg = (
        f"✅ **{fund_name}** — detected `{date_col}` → Date, `{price_col}` → Price  "
...
# ─────────────────────────────────────────────────────────────────────────────
def render_stats_table(stats_map: dict):
    """Render rolling-window analytics as styled DataFrame."""
    rows = []
    for fund, stats in stats_map.items():
        for label in WINDOWS:
            w = stats.get(label) if stats else None
            rows.append(
                {
                    "Fund": fund,
                    "Window": label,
                    "Cumulative Return": f"{w['cumulative_return']:.2%}" if w else "—",
                    "Annualized Volatility": f"{w['annualized_volatility']:.2%}" if w else "—",
                    "Obs.": f"{w['n_obs']:,}" if w else "—",
                    "Freq (Ann.)": str(w["freq"]) if w else "—",
                }
            )
    if rows:
        st.dataframe(
...
with st.sidebar:
    st.markdown(
        ...
        "<p style='font-size:0.78rem;color:#6b7f90;margin-top:0;'>Upload CSV files to begin</p>",
        unsafe_allow_html=True,
    )
    st.markdown("---")

    uploaded_files = st.file_uploader(
        "Upload Fund CSV files",
        type=["csv"],
        accept_multiple_files=True,
        help="Upload one or more fund CSVs. Column headers are auto-detected.",
    )
...
if not uploaded_files:
    st.markdown(
        ...
        "Column headers are automatically detected — no manual mapping needed.</p>"
        "</div>",
        unsafe_allow_html=True,
    )
    st.stop()
...
with tab_stats:
    st.markdown("##### Rolling-Window Performance & Volatility")
    st.caption(
        "Windows calculated from the most recent date in each fund's dataset. "
        "Volatility = σ(periodic returns) × √annualisation_factor."
    )
    render_stats_table(stats_map)

    st.markdown("---")
    st.markdown("##### Per-Fund Calendar Year Returns")
    for fund in funds:
        stats = stats_map.get(fund)
        if not stats or not stats.get("calendar_years"):
            continue
        cy = stats["calendar_years"]
        cal_df = pd.DataFrame(
            [
                ...
                for y, r in sorted(cy.items())
            ]
        )
        color = fund_color_map.get(fund, "#00d4aa")
        st.markdown(
            f"<div style='border-left:3px solid {color}; padding-left:0.7rem;"
            f"margin-bottom:0.4rem;'><b>{fund}</b></div>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown("""
- **Normalized Price**: Each fund's price series divided by its first available price (base = 1.0).
- **Annualized Volatility**: `std(daily_returns) × √252` for daily data, `×√52` weekly, `×√12` monthly.
  Frequency is auto-detected from median inter-observation gaps.
- **Cumulative Return**: `(last_price / first_price_in_window) − 1`
- **Calendar Year Return**: `(year_end_price / year_start_price) − 1` for each full year.
- **Black Swan Regions**: Plotted as translucent vertical rectangles via Plotly's `add_vrect`.
    """)
