...
with tab_div:
    if not all_divs:
        ...
        with st.expander("Full Dividend History Table"):
            ...
            # Look up NAV on each ex-date for monthly yield calculation
            ...
            # Monthly Yield % = distribution ÷ NAV × 12 (annualised)
            ...
            display_div["NAV_str"] = display_div["Price"].map(lambda v: f"USD {v:.4f}" if pd.notna(v) else "—")
            display_div["Yield_str"] = display_div["Monthly_Yield"].map(lambda v: f"{v:.2%}" if pd.notna(v) else "—")
            st.dataframe(
                display_div[["Fund_Name", "Ex_Date", "Ex_Date_str", "Year", "Distribution_str", "NAV_str", "Yield_str"]]
                .sort_values(["Fund_Name", "Ex_Date"], ascending=[True, False])  # datetime sort — correct order
                .drop(columns=["Ex_Date"])
                .rename(columns={
                    "Fund_Name": "Fund", "Ex_Date_str": "Ex-Date", "Year": "Year",
                    "Distribution_str": "Distribution", "NAV_str": "Unit Price", "Yield_str": "Monthly Yield %",
                }),
                use_container_width=True,
                hide_index=True,
            )
