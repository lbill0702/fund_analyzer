...
.sb-tagline {
  ...
  color: var(--text3) !important; margin-top: 0.15rem;
}

.sb-section { padding: 1.1rem 1.25rem 0.5rem; }
.sb-label {
  font-size: 0.58rem !important; font-weight: 700 !important;
  letter-spacing: 0.15em !important; text-transform: uppercase !important;
  color: var(--text3) !important; margin-bottom: 0.6rem !important;
...
.sb-footer p {
  font-size: 0.64rem !important; color: var(--text3) !important;
...
[data-testid="stCheckbox"] > label {
  font-size: 0.78rem !important; gap: 0.5rem !important;
...
.stat-card.positive { border-left: 2px solid var(--positive); }
.stat-card.negative { border-left: 2px solid var(--negative); }
.stat-card .label {
  font-size: 0.6rem; font-weight: 700;
...
.stat-card .meta {
  ...
  margin-top: 0.4rem; font-family: var(--font-mono) !important;
...
.stButton button {
  ...
  transition: all 0.15s !important; letter-spacing: 0 !important;
}
...
[data-testid="stExpander"] summary {
  ...
  color: var(--text2) !important; padding: 0.65rem 0.9rem !important;
  letter-spacing: 0.01em !important;
}
...
[data-testid="stCaptionContainer"] {
  font-size: 0.7rem !important; color: var(--text3) !important;
...
.fmt-chip {
  ...
  font-family: var(--font-mono) !important;
  font-size: 0.65rem; color: var(--text2) !important;
}
...
.ingest-ok {
  ...
  background: var(--positive-bg); border: 1px solid rgba(0,200,150,0.18);
  border-radius: var(--radius-sm); padding: 0.55rem 0.8rem;
  margin-bottom: 0.4rem; width: 100%;
}
.ingest-err {
  display: inline-flex; align-items: flex-start; gap: 0.5rem;
  background: var(--negative-bg); border: 1px solid rgba(255,77,106,0.18);
  border-radius: var(--radius-sm); padding: 0.55rem 0.8rem;
  margin-bottom: 0.4rem; width: 100%;
}
.ingest-icon { font-size: 0.75rem; margin-top: 0.05rem; flex-shrink: 0; }
.ingest-text { font-size: 0.72rem !important; line-height: 1.7 !important; flex: 1; }
.ingest-text strong { color: var(--text) !important; font-weight: 600; }
.ingest-text em { color: var(--text3) !important; font-style: normal; font-family: var(--font-mono) !important; font-size: 0.67rem !important; }
...
.bs-date {
  font-size: 0.63rem; font-family: var(--font-mono) !important;
  margin-top: 0.15rem; opacity: 0.65;
}
...
for i, fund in enumerate(funds[:4]):
    ...
    with col_metrics[i]:
        pos_neg = "positive" if total_ret >= 0 else "negative"
        sign = "+" if total_ret >= 0 else ""
        date_range = f"{fd['Date'].min().strftime('%b %Y')} – {fd['Date'].max().strftime('%b %Y')}"
        n_obs = len(fd)
        color = fund_color_map.get(fund, "#00c896")
        st.markdown(
            f"<div class='stat-card {pos_neg}' style='border-left-color:{color}'>"
            f"<div class='label'>{fund[:30]}{'…' if len(fund)>30 else ''}</div>"
            f"<div class='value {pos_neg}'>{sign}{total_ret:.2%}</div>"
            f"<div class='meta'>{date_range} &nbsp;·&nbsp; {n_obs:,} obs</div>"
            f"</div>",
            unsafe_allow_html=True,
        )
        ...
        for fund in combined_div_all["Fund_Name"].unique():
            ...
            with col1:
                st.markdown(
                    ...
                    f"<div class='value' style='color:{color};font-size:1.3rem'>USD {latest['Distribution']:.4f}</div>"
                    f"<div class='meta'>{latest['Ex_Date'].strftime('%d %b %Y')}</div></div>",
                    unsafe_allow_html=True,
                )
            with col2:
                ...
                st.markdown(
                    ...
                    f"<div class='value' style='color:{color};font-size:1.3rem'>{yield_str}</div>"
                    f"<div class='meta'>NAV {nav_str}</div></div>",
                    unsafe_allow_html=True,
                )
            with col3:
                st.markdown(
                    ...
                    f"<div class='value' style='color:{color};font-size:1.3rem'>USD {trailing12:.4f}</div>"
                    f"<div class='meta'>per unit</div></div>",
                    unsafe_allow_html=True,
                )
            with col4:
                count = len(fd)
                st.markdown(
                    ...
                    f"<div class='value' style='color:{color};font-size:1.3rem'>{count}</div>"
                    f"<div class='meta'>since inception</div></div>",
                    unsafe_allow_html=True,
                )
            with col5:
                ...
                st.markdown(
                    ...
                    f"<div class='value' style='color:{color};font-size:1.3rem'>USD {avg:.4f}</div>"
                    f"<div class='meta'>all-time mean</div></div>",
                    unsafe_allow_html=True,
                )
